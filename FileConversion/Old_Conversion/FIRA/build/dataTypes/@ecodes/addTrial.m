...
% For each name string in <names>, need matching index into 
% FIRA.ecodes.name. ismember could provide, but it's slow.
% strcmp and a loop are faster
memberi = [];
for ii = 1:length(names)

    % index of FIRA.ecodes.name matching <names>, if any
    memberi = [memberi, find(strcmp(FIRA.ecodes.name,names{ii}))];

end

% put <data> elements into indicated columns of FIRA.ecodes.data
FIRA.ecodes.data(end,memberi) = data;
