function varargout = plotGUI_psych_estim(varargin)
% GUI for the caudate microstim experiment
% Use spmLDdotsEstim under Matlab/Long/Analysis
%
% Created 01/20/12 by Taka Doi from plotGUI_psychometric 
%----- 
...
gsGUI_selectByUniqueID('setf', handles.outcomemenu, 'OLscore');
...
gsGUI_ecodeTimesByName('setf', handles.dotoffmenu, {'sac_on', 'ins_off'}); % TD 01/21/12 I'm not sure what 'ins_off' is...
...
% set fEstim
set(handles.estimmenu, 'Value', 1, 'String', { ...
    'estim_type',   ...
    });
...
handles.select = gsGUI_selectTrials('initf', ...
    handles.selectbutton, @plotGUI_psych_estim);

% set up the "save struct" ... see gsGUI_saveSettings for details
handles = gsGUI_saveSettings('setf', handles.figure1, handles, 'psych_estim', ...
    {...
    'menu',     'cohmenu';     ...
    'menu',     'outcomemenu'; ...
    'menu',     'dirmenu';     ...
...
if ~isempty(trials)
    ...
    if length(tstart) == length(tstop)
        ...
        if any(Lgood)
            ...
            % get estim_type, selection arrays
            [Lest, ests] = gsGUI_selectByUniqueID('getf', handles.estimmenu, trials, 12);
...
%             fDebugFIRA;
            tmp_nTrial = sum(Lout*outs==1 | Lout*outs==0);
            set(handles.textNtrials, 'String', [num2str(tmp_nTrial)]);

            % check for cohs, dirs
            if size(cohs, 1) > 0 && size(dirs, 1) == 2 && size(ests, 1) == 2
                % set flag
                cont = true;
                for iEst = 1:2
                    Lgood = sum(Lcoh, 2)==1 & sum(Lout, 2)==1 & sum(Ldir, 2)==1 & Lest(:,iEst);
                    [trials_e{iEst} view_times_e{iEst} Lcoh_e{iEst} Lout_e{iEst}...
                     Ldir_e{iEst} Lcor_e{iEst} Lerr_e{iEst} Lchc_e{iEst} c_cor_e{iEst} c_err_e{iEst} ...
                     c_nc_e{iEst} c_bf_e{iEst}] =...
                        fExtract_trials_estim(Lgood, Lcoh, Lout, Ldir, outs, trials, view_times);
                end
            end
        end
    end
...
if size(cohs, 1) > 0 && size(dirs, 1) == 2 && size(ests, 1) == 2
    % Try to encapsulate the following process.
    % Make two sets of variables depending on estim condition (Lest, ests)
    % "ests" should be [0 2]'. Thus, The 1st column of Lest is 1 for
    % estim-off trials.
    co_est = {'b','r'};
    for iEst=1:2
        fmake_plot(Lcoh_e{iEst},Lchc_e{iEst},Ldir_e{iEst},Lcor_e{iEst},...
                   Lerr_e{iEst},Lout_e{iEst},handles,cont,...
                   c_cor_e{iEst},c_err_e{iEst},c_nc_e{iEst},c_bf_e{iEst},...
                   trials_e{iEst},show_pmf,show_summary,min_time,max_time,cohs,iEst,co_est{iEst});
    end
else
    disp([mfilename, ': Not enough trials. size cohs = '...
          ,num2str(size(cohs,1)),' size dirs = ',num2str(size(dirs,1))]);
%     keyboard;
end

catch
     com = lasterror;
     disp([com.message,' @Line',num2str([com.stack.line])]);
     keyboard;
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function fmake_plot(Lcoh,Lchc,Ldir,Lcor,Lerr,Lout,handles,cont,c_cor,c_err,c_nc,c_bf,...
                    trials,show_pmf,show_summary,min_time,max_time,cohs,iEst,co_est)
try
% handles
if iEst==1 % No stim
    myhandles.textnb      = handles.textnb;
    myhandles.textbias    = handles.textbias;
    myhandles.textcorrect = handles.textcorrect;
    myhandles.textlapse   = handles.textlapse;
    myhandles.textparams  = handles.textparams;
elseif iEst==2
    myhandles.textnb      = handles.textnb1;
    myhandles.textbias    = handles.textbias1;
    myhandles.textcorrect = handles.textcorrect1;
    myhandles.textlapse   = handles.textlapse1;
    myhandles.textparams  = handles.textparams1;
end
                
% nothing found .. clean up and go
if ~cont
    set(myhandles.textnb,      'String', 'n/a');
    set(myhandles.textbias,    'String', 'n/a');
    set(myhandles.textcorrect, 'String', 'n/a');
    set(myhandles.textlapse,   'String', 'n/a');
    return
end
...
set(myhandles.textnb, 'String', sprintf('%d / %d', ...
...
err = sum(Lerr);
if cor + err > 0
    set(myhandles.textcorrect, 'String', ...
...
switch fit
    ...
    case 2
        ...
        % show lapse rate
        set(myhandles.textlapse, 'String', sprintf('%.1f', lapse*100));
    ...
    case 3
        ...
        % show lapse rate
        set(myhandles.textlapse, 'String', sprintf('%.1f / %.1f', lapse*100));
    ...
    case 4
        ...
        % show lapse rate
        set(myhandles.textlapse, 'String', sprintf('%.1f / %.1f', fits(1:2)*100));
...
if show_summary
    ...
    if binshow == 1 || binshow-1 > size(dat, 3) % 'all'
      ...
      % Not separate histograms with respect to choice but estim
      % condition. TD 01/23/12
      % 
      nFin = sum(dat(:,1:2),2);
      if iEst==1
          x_bar = [1:length(cohs)]'-0.2;
      elseif iEst==2
          x_bar = [1:length(cohs)]'+0.2;
      end
      tmp_bar = bar(x_bar, nFin,'FaceColor',co_est,'EdgeColor','none','barwidth',0.5);
    else
    ...
    set(handles.summaryaxis, 'XTick',[1:length(cohs)]);
...
if show_pmf
    ...
    for dd = 1:size(dat, 3)
        ...
        hh(dd) = plot(handles.pmfaxis, xx, dat(:,1,dd)./den*100, co{dd}, ...
            'MarkerSize', 20);
        set(hh(dd), 'color',co_est,'MarkerEdgeColor',co_est,'MarkerFaceColor',co_est);
    end
    ...
    switch fit
        ...
        case 1 % none
            set(myhandles.textparams, 'String', '');
        ...
        case 3 % simple logistic
            ...
            if timeflg
                ...
                % Don't plot when estim is on with estimRevCorr
                iRevCorr = get(handles.togglebutton1,'Value');
                if (iEst==1) || (~iRevCorr)
                    % plotting versus coh
                    ...
                    for ff = 1:num_bins
                        plot(xbase, 100*logist_val(xcomp, lapse(1), lapse(2), fits(1:2, ff)), ...
                            co{ff}, 'LineStyle', '-', 'lineWidth', 2, 'Marker', 'none', 'color',co_est);
                    end

                    % plot a line at the lapses
                    plot([minx maxx], 100*[lapse(1) lapse(1)],     '--', 'color', co_est);
                    plot([minx maxx], 100-100*[lapse(2) lapse(2)], '--', 'color', co_est);
                end
            end
...
%% TD 
function [trials view_times Lcoh Lout Ldir Lcor Lerr Lchc c_cor c_err c_nc c_bf cohs] =...
    fExtract_trials_estim(Lgood, Lcoh, Lout, Ldir, outs, trials, view_times)
