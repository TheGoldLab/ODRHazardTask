function [sacs_] = findSaccadesADPODR(Xin, Yin, store_rate, num_sacs, show_fig)
% function [sacs_] = findSaccadesADPODR(Xin, Yin, store_rate, num_sacs, show_fig)
%
...
while size(sacs_, 1) < num_sacs
    ...
    if isempty(sac_begin) || isempty(decel)
        ...
        % Changed by jig 3/2/22
        sac_end = decel-5 + max(...
            find(acc(decel+1:end-1) > -A_MIN, 1), ...
            find(vels(decel:end-2) <= 0.005, 1));
        %         sac_end = decel-5 + max(...
        %             find(acc(decel+1:end-1) > -A_MIN, 1), ...
        %             find(vels(decel:end-2) <= (VI_MIN/2) | ...
