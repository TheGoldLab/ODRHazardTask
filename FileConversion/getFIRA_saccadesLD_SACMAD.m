function [sacs_, bf_] = getFIRA_saccadesLD_SACMAD(trial, num_saccades, recal, ...
    heye, veye, fpoff, fpx, fpy, max_time)
% modified from getFIRA_saccadesLD.m, LD 2009-08-23
% use signals -100 ms to max_time after SACMAD ecode time to get actual saccade data
    ...
    if any(eyeX) && ~isempty(FIRA.analog.data(trial, eyeX).values) && ...
        ...
        if recal
            ...
            FIRA.analog.data(trial, eyeX).values = ...
                FIRA.analog.data(trial, eyeX).values - ...
                mean(FIRA.analog.data(trial, eyeX).values(fpoff_samp-100:fpoff_samp)) + ...
                FIRA.ecodes.data(trial, strcmp(fpx, FIRA.ecodes.name));

            FIRA.analog.data(trial, eyeY).values = ...
                FIRA.analog.data(trial,eyeY).values - ...
                mean(FIRA.analog.data(trial,eyeY).values(fpoff_samp-100:fpoff_samp)) + ...
